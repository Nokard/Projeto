				...
				try {
					...
					if (rs.next()) {
						if(table.getSelectedRow() != -1) {
							int yes = JOptionPane.showConfirmDialog(null, "Voc̻ realmente deseja ATUALIZAR esse Cliente ?","ATUALIZAR",JOptionPane.YES_NO_OPTION);	
							if(yes == 0) {
		...
		button_2.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					if (rs.next()) {
						if(table.getSelectedRow() != -1) {
							int yes = JOptionPane.showConfirmDialog(null, "Voc̻ realmente deseja DELETAR esse Cliente ?","DELETE",JOptionPane.YES_NO_OPTION);
							if (yes == 0) {
								try {
									DadoPf dadoPf = new DadoPf();
									ContatoDaoCliente daoPf = new ContatoDaoCliente();
